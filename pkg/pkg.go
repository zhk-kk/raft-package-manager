...
func CompileTemplate(templatePath string, w io.Writer) error {
	...
	if err := filepath.WalkDir(templatePath, func(p string, d fs.DirEntry, err error) error {
		...
		if isMetadata {
			fh.Comment = "RaftPM package metadata"
		}
		// Preserve attributes of the file.
		fh.SetMode(fileInfo.Mode())
	...
	// Add a comment.
	zipW.SetComment("Package generated by the raft package manager")
